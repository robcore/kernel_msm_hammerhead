...
/* Thermal throttling indicator, 0 = is not throttled, 1 = low, 2 = mid, 3 = max */
int throttling_indicator = 0;
EXPORT_SYMBOL_GPL(throttling_indicator);

/* Save the CPU max freq before throttling */
static int saved_max = 0;
...
static void __ref check_temp(struct work_struct *work)
{
	...
	for_each_possible_cpu(cpu) {
		...
		update_stats();
		start_stats(throttling_indicator);
		...
		for_each_possible_cpu(cpu) {
			...
			if (cpu_policy) {
				if (cpu_policy->max < cpu_policy->cpuinfo.max_freq)
					update_cpu_max_freq(cpu_policy, cpu, saved_max);
				cpufreq_cpu_put(cpu_policy);
			}
		}
	}
